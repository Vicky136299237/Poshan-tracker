...
# Loading dataset
df = pd.read_csv(r"C:\Users\V\Desktop\Python project\mapped_poshan_tracker_updated.csv")
...
#4. Creating new column: Children Coverage Ratio
...
# ------------------ Z-test ------------------
# Testing if the mean of 'lact_mothers' is significantly different from 0
sample = df['lact_mothers'].dropna()
population_mean = 0
z_stat, p_val = stats.ttest_1samp(sample, population_mean)
print("Z-test result:")
print(f"Z-statistic: {z_stat:.2f}, P-value: {p_val:.4f}")

# ------------------ T-test ------------------
# Comparing the means of 'pregnant_women' and 'lact_mothers'
group1 = df['pregnant_women'].dropna()
group2 = df['lact_mothers'].dropna()
t_stat, p_value = stats.ttest_ind(group1, group2, equal_var=False)
print("\nT-test result:")
print(f"T-statistic: {t_stat:.2f}, P-value: {p_value:.4f}")

# ------------------ Chi-Square Test ------------------
# Testing independence between 'state_code' and 'project'
contingency_table = pd.crosstab(df['state_code'], df['project'])
...
# Outlier detection for 'children_0_6_months'
Q1 = df['children_0_6_months'].quantile(0.25)
Q3 = df['children_0_6_months'].quantile(0.75)
IQR = Q3 - Q1
lower_bound = Q1 - 1.5 * IQR
upper_bound = Q3 + 1.5 * IQR

outliers = df[(df['children_0_6_months'] < lower_bound) | (df['children_0_6_months'] > upper_bound)]
print("\nOutliers detected using IQR method in 'children_0_6_months':")
print(outliers[['district_name', 'children_0_6_months']])
